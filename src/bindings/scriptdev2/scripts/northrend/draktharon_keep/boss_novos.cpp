...
uint32 m_uiRandomSummoningSpell[5] = {SPELL_SUMMON_RISEN_SHADOWCASTER, SPELL_SUMMON_RISEN_SHADOWCASTER, 
SPELL_SUMMON_SUMMON_FETID_TROLL_CORPSE, SPELL_SUMMON_SUMMON_FETID_TROLL_CORPSE, SPELL_SUMMON_HULKING_CORPSE};
...
struct MANGOS_DLL_DECL boss_novosAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        m_uiSummonCrystalChanneler_Timer = 15000/3;
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiPhase == PHASE_SUMMONING)
        {
            ...
            if (m_uiSummonRandomAdd_Timer <= uiDiff)
            {
                if (m_pInstance)
                    ...
                    if (Creature* pSummonTarget = m_pInstance->instance->GetCreature(m_uiSummonTargetGUID[0]))
                        pSummonTarget->CastSpell(pSummonTarget, m_uiRandomSummoningSpell[urand(0, 4)], true);

                m_uiSummonRandomAdd_Timer = urand(4000, 7000);
            }
            else
            ...
            if (m_uiSummonCrystalChanneler_Timer < uiDiff)
            {
                ...
                m_uiSummonCrystalChanneler_Timer = 25000/3;
            }
            else
        ...
        {               
            ...
            {
                DoCastSpellIfCan(m_creature->getVictim(), m_bIsRegularMode ? SPELL_MISERY : SPELL_MISERY_H);
            ...
            {
                if (Unit* pTarget = m_creature->SelectAttackingTarget(ATTACKING_TARGET_RANDOM, 0))
                    DoCastSpellIfCan(pTarget, m_bIsRegularMode ? SPELL_BLIZZARD : SPELL_BLIZZARD_H);
